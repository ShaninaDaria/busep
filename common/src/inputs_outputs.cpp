#include "../../common/hdr/inputs_outputs.h"


InputsOutputs::InputsOutputs()
{

}

_inputs InputsOutputs::getInputs() const
{
    return inputs;
}

_inputs InputsOutputs::initInputs()
{
    inputs.setInput1(input_state::no_input_state);
    inputs.setInput2(input_state::no_input_state);
    inputs.setInput3(input_state::no_input_state);
    inputs.setInput4(input_state::no_input_state);

    inputs.setInput5(input_state::no_input_state);
    inputs.setInput6(input_state::no_input_state);
    inputs.setInput7(input_state::no_input_state);
    inputs.setInput8(input_state::no_input_state);

    inputs.setInput9(input_state::no_input_state);
    inputs.setInput10(input_state::no_input_state);
    inputs.setInput11(input_state::no_input_state);
    inputs.setInput12(input_state::no_input_state);

    inputs.setInput13(input_state::no_input_state);
    inputs.setInput14(input_state::no_input_state);
    inputs.setInput15(input_state::no_input_state);
    inputs.setInput16(input_state::no_input_state);

    inputs.setInput17(input_state::no_input_state);
    inputs.setInput18(input_state::no_input_state);
    inputs.setInput19(input_state::no_input_state);
    inputs.setInput20(input_state::no_input_state);

    inputs.setInput21(input_state::no_input_state);
    inputs.setInput22(input_state::no_input_state);
    inputs.setInput23(input_state::no_input_state);
    inputs.setInput24(input_state::no_input_state);

    inputs.setInput25(input_state::no_input_state);
    inputs.setInput26(input_state::no_input_state);
    inputs.setInput27(input_state::no_input_state);
    inputs.setInput28(input_state::no_input_state);

    inputs.setInput29(input_state::no_input_state);
    inputs.setInput30(input_state::no_input_state);
    inputs.setInput31(input_state::no_input_state);
    inputs.setInput32(input_state::no_input_state);

    inputs.setInput33(input_state::no_input_state);
    inputs.setInput34(input_state::no_input_state);
    inputs.setInput35(input_state::no_input_state);
    inputs.setInput36(input_state::no_input_state);

    inputs.setInput37(input_state::no_input_state);
    inputs.setInput38(input_state::no_input_state);
    inputs.setInput39(input_state::no_input_state);
    inputs.setInput40(input_state::no_input_state);

    inputs.setInput41(input_state::no_input_state);
    inputs.setInput42(input_state::no_input_state);
    inputs.setInput43(input_state::no_input_state);
    inputs.setInput44(input_state::no_input_state);

    inputs.setInput45(input_state::no_input_state);
    inputs.setInput46(input_state::no_input_state);
    inputs.setInput47(input_state::no_input_state);
    inputs.setInput48(input_state::no_input_state);

    inputs.setInput49(input_state::no_input_state);
    inputs.setInput50(input_state::no_input_state);
    inputs.setInput51(input_state::no_input_state);
    inputs.setInput52(input_state::no_input_state);

    inputs.setInput53(input_state::no_input_state);
    inputs.setInput54(input_state::no_input_state);
    inputs.setInput55(input_state::no_input_state);
    inputs.setInput56(input_state::no_input_state);

    inputs.setInput57(input_state::no_input_state);
    inputs.setInput58(input_state::no_input_state);
    inputs.setInput59(input_state::no_input_state);
    inputs.setInput60(input_state::no_input_state);

    inputs.setInput61(input_state::no_input_state);
    inputs.setInput62(input_state::no_input_state);
    inputs.setInput63(input_state::no_input_state);
    inputs.setInput64(input_state::no_input_state);

    inputs.setInput65(input_state::no_input_state);
    inputs.setInput66(input_state::no_input_state);
    inputs.setInput67(input_state::no_input_state);
    inputs.setInput68(input_state::no_input_state);

    inputs.setInput69(input_state::no_input_state);
    inputs.setInput70(input_state::no_input_state);
    inputs.setInput71(input_state::no_input_state);
    inputs.setInput72(input_state::no_input_state);

    inputs.setInput73(input_state::no_input_state);
    inputs.setInput74(input_state::no_input_state);
    inputs.setInput75(input_state::no_input_state);
    inputs.setInput76(input_state::no_input_state);

    inputs.setInput77(input_state::no_input_state);
    inputs.setInput78(input_state::no_input_state);
    inputs.setInput79(input_state::no_input_state);
    inputs.setInput80(input_state::no_input_state);

    inputs.setInput81(input_state::no_input_state);
    inputs.setInput82(input_state::no_input_state);
    inputs.setInput83(input_state::no_input_state);
    inputs.setInput84(input_state::no_input_state);

    inputs.setInput85(input_state::no_input_state);
    inputs.setInput86(input_state::no_input_state);
    inputs.setInput87(input_state::no_input_state);
    inputs.setInput88(input_state::no_input_state);

    inputs.setInput89(input_state::no_input_state);
    inputs.setInput90(input_state::no_input_state);
    inputs.setInput91(input_state::no_input_state);
    inputs.setInput92(input_state::no_input_state);

    inputs.setInput93(input_state::no_input_state);
    inputs.setInput94(input_state::no_input_state);
    inputs.setInput95(input_state::no_input_state);
    inputs.setInput96(input_state::no_input_state);

    inputs.setInput97(input_state::no_input_state);
    inputs.setInput98(input_state::no_input_state);
    inputs.setInput99(input_state::no_input_state);
    inputs.setInput100(input_state::no_input_state);

    inputs.setInput101(input_state::no_input_state);
    inputs.setInput102(input_state::no_input_state);
    inputs.setInput103(input_state::no_input_state);
    inputs.setInput104(input_state::no_input_state);

    inputs.setInput105(input_state::no_input_state);
    inputs.setInput106(input_state::no_input_state);
    inputs.setInput107(input_state::no_input_state);
    inputs.setInput108(input_state::no_input_state);

    inputs.setInput109(input_state::no_input_state);
    inputs.setInput110(input_state::no_input_state);
    inputs.setInput111(input_state::no_input_state);
    inputs.setInput112(input_state::no_input_state);

    inputs.setInput113(input_state::no_input_state);
    inputs.setInput114(input_state::no_input_state);
    inputs.setInput115(input_state::no_input_state);
    inputs.setInput116(input_state::no_input_state);

    inputs.setInput117(input_state::no_input_state);
    inputs.setInput118(input_state::no_input_state);
    inputs.setInput119(input_state::no_input_state);
    inputs.setInput120(input_state::no_input_state);

    inputs.setInput121(input_state::no_input_state);
    inputs.setInput122(input_state::no_input_state);
    inputs.setInput123(input_state::no_input_state);
    inputs.setInput124(input_state::no_input_state);

    return inputs;
}

_inputs InputsOutputs::changeInputs()
{
    inputs.setInput1(input_state::no_signal_27v);
    inputs.setInput2(input_state::is_signal_27v);
    inputs.setInput3(input_state::no_input_state);
    inputs.setInput4(input_state::no_signal_27v);

    inputs.setInput5(input_state::no_signal_27v);
    inputs.setInput6(input_state::is_signal_27v);
    inputs.setInput7(input_state::no_input_state);
    inputs.setInput8(input_state::no_signal_27v);

    inputs.setInput9(input_state::no_signal_27v);
    inputs.setInput10(input_state::is_signal_27v);
    inputs.setInput11(input_state::no_input_state);
    inputs.setInput12(input_state::no_signal_27v);

    inputs.setInput13(input_state::no_signal_27v);
    inputs.setInput14(input_state::is_signal_27v);
    inputs.setInput15(input_state::no_input_state);
    inputs.setInput16(input_state::no_signal_27v);

    inputs.setInput17(input_state::no_signal_27v);
    inputs.setInput18(input_state::is_signal_27v);
    inputs.setInput19(input_state::no_input_state);
    inputs.setInput20(input_state::no_signal_27v);

    inputs.setInput21(input_state::no_signal_27v);
    inputs.setInput22(input_state::is_signal_27v);
    inputs.setInput23(input_state::no_input_state);
    inputs.setInput24(input_state::no_signal_27v);

    inputs.setInput25(input_state::no_signal_27v);
    inputs.setInput26(input_state::is_signal_27v);
    inputs.setInput27(input_state::no_input_state);
    inputs.setInput28(input_state::no_signal_27v);

    inputs.setInput29(input_state::no_signal_27v);
    inputs.setInput30(input_state::is_signal_27v);
    inputs.setInput31(input_state::no_input_state);
    inputs.setInput32(input_state::no_signal_27v);

    inputs.setInput33(input_state::no_signal_27v);
    inputs.setInput34(input_state::is_signal_27v);
    inputs.setInput35(input_state::no_signal_27v);
    inputs.setInput36(input_state::is_signal_27v);

    inputs.setInput37(input_state::no_signal_27v);
    inputs.setInput38(input_state::is_signal_27v);
    inputs.setInput39(input_state::no_signal_27v);
    inputs.setInput40(input_state::is_signal_27v);

    inputs.setInput41(input_state::no_signal_27v);
    inputs.setInput42(input_state::is_signal_27v);
    inputs.setInput43(input_state::no_signal_27v);
    inputs.setInput44(input_state::is_signal_27v);

    inputs.setInput45(input_state::no_signal_27v);
    inputs.setInput46(input_state::is_signal_27v);
    inputs.setInput47(input_state::no_signal_27v);
    inputs.setInput48(input_state::is_signal_27v);

    inputs.setInput49(input_state::no_signal_27v);
    inputs.setInput50(input_state::is_signal_27v);
    inputs.setInput51(input_state::no_signal_27v);
    inputs.setInput52(input_state::is_signal_27v);

    inputs.setInput53(input_state::no_signal_27v);
    inputs.setInput54(input_state::is_signal_27v);
    inputs.setInput55(input_state::no_signal_27v);
    inputs.setInput56(input_state::is_signal_27v);

    inputs.setInput57(input_state::no_signal_27v);
    inputs.setInput58(input_state::is_signal_27v);
    inputs.setInput59(input_state::no_signal_27v);
    inputs.setInput60(input_state::is_signal_27v);

    inputs.setInput61(input_state::no_signal_27v);
    inputs.setInput62(input_state::is_signal_27v);
    inputs.setInput63(input_state::no_signal_27v);
    inputs.setInput64(input_state::is_signal_27v);

    inputs.setInput65(input_state::no_signal_27v);
    inputs.setInput66(input_state::is_signal_27v);
    inputs.setInput67(input_state::no_signal_27v);
    inputs.setInput68(input_state::is_signal_27v);

    inputs.setInput69(input_state::no_signal_27v);
    inputs.setInput70(input_state::is_signal_27v);
    inputs.setInput71(input_state::no_signal_27v);
    inputs.setInput72(input_state::is_signal_27v);

    inputs.setInput73(input_state::no_signal_27v);
    inputs.setInput74(input_state::is_signal_27v);
    inputs.setInput75(input_state::no_signal_27v);
    inputs.setInput76(input_state::is_signal_27v);

    inputs.setInput77(input_state::no_signal_27v);
    inputs.setInput78(input_state::is_signal_27v);
    inputs.setInput79(input_state::no_signal_27v);
    inputs.setInput80(input_state::is_signal_27v);

    inputs.setInput81(input_state::no_signal_27v);
    inputs.setInput82(input_state::is_signal_27v);
    inputs.setInput83(input_state::no_signal_27v);
    inputs.setInput84(input_state::is_signal_27v);

    inputs.setInput85(input_state::no_signal_27v);
    inputs.setInput86(input_state::is_signal_27v);
    inputs.setInput87(input_state::no_signal_27v);
    inputs.setInput88(input_state::is_signal_27v);

    inputs.setInput89(input_state::no_signal_27v);
    inputs.setInput90(input_state::is_signal_27v);
    inputs.setInput91(input_state::no_signal_27v);
    inputs.setInput92(input_state::is_signal_27v);

    inputs.setInput93(input_state::no_signal_27v);
    inputs.setInput94(input_state::is_signal_27v);
    inputs.setInput95(input_state::no_signal_27v);
    inputs.setInput96(input_state::is_signal_27v);

    inputs.setInput97(input_state::no_signal_27v);
    inputs.setInput98(input_state::is_signal_27v);
    inputs.setInput99(input_state::no_signal_27v);
    inputs.setInput100(input_state::is_signal_27v);

    inputs.setInput101(input_state::no_signal_27v);
    inputs.setInput102(input_state::is_signal_27v);
    inputs.setInput103(input_state::no_signal_27v);
    inputs.setInput104(input_state::is_signal_27v);

    inputs.setInput105(input_state::no_signal_27v);
    inputs.setInput106(input_state::is_signal_27v);
    inputs.setInput107(input_state::no_signal_27v);
    inputs.setInput108(input_state::is_signal_27v);

    inputs.setInput109(input_state::no_signal_27v);
    inputs.setInput110(input_state::is_signal_27v);
    inputs.setInput111(input_state::no_signal_27v);
    inputs.setInput112(input_state::is_signal_27v);

    inputs.setInput113(input_state::no_signal_27v);
    inputs.setInput114(input_state::is_signal_27v);
    inputs.setInput115(input_state::no_signal_27v);
    inputs.setInput116(input_state::is_signal_27v);

    inputs.setInput117(input_state::no_signal_27v);
    inputs.setInput118(input_state::is_signal_27v);
    inputs.setInput119(input_state::no_signal_27v);
    inputs.setInput120(input_state::is_signal_27v);

    inputs.setInput121(input_state::no_signal_27v);
    inputs.setInput122(input_state::is_signal_27v);
    inputs.setInput123(input_state::no_signal_27v);
    inputs.setInput124(input_state::is_signal_27v);

    return inputs;
}

void InputsOutputs::setInputs(const _inputs &value)
{
    inputs = value;
}

_outputs InputsOutputs::initOutputs()
{
    outputs.setOutput1(no_output_state);
    outputs.setOutput2(no_output_state);
    outputs.setOutput3(no_output_state);
    outputs.setOutput4(no_output_state);

    outputs.setOutput5(no_output_state);
    outputs.setOutput6(no_output_state);
    outputs.setOutput7(no_output_state);
    outputs.setOutput8(no_output_state);

    outputs.setOutput9(no_output_state);
    outputs.setOutput10(no_output_state);
    outputs.setOutput11(no_output_state);
    outputs.setOutput12(no_output_state);

    outputs.setOutput13(no_output_state);
    outputs.setOutput14(no_output_state);
    outputs.setOutput15(no_output_state);
    outputs.setOutput16(no_output_state);

    outputs.setOutput17(no_output_state);
    outputs.setOutput18(no_output_state);
    outputs.setOutput19(no_output_state);
    outputs.setOutput20(no_output_state);

    outputs.setOutput21(no_output_state);
    outputs.setOutput22(no_output_state);
    outputs.setOutput23(no_output_state);
    outputs.setOutput24(no_output_state);

    outputs.setOutput25(no_output_state);
    outputs.setOutput26(no_output_state);
    outputs.setOutput27(no_output_state);
    outputs.setOutput28(no_output_state);

    outputs.setOutput29(no_output_state);
    outputs.setOutput30(no_output_state);
    outputs.setOutput31(no_output_state);
    outputs.setOutput32(no_output_state);

    outputs.setOutput33(no_output_state);
    outputs.setOutput34(no_output_state);
    outputs.setOutput35(no_output_state);
    outputs.setOutput36(no_output_state);

    outputs.setOutput37(no_output_state);
    outputs.setOutput38(no_output_state);
    outputs.setOutput39(no_output_state);
    outputs.setOutput40(no_output_state);

    outputs.setOutput41(no_output_state);
    outputs.setOutput42(no_output_state);
    outputs.setOutput43(no_output_state);
    outputs.setOutput44(no_output_state);

    outputs.setOutput45(no_output_state);
    outputs.setOutput46(no_output_state);
    outputs.setOutput47(no_output_state);
    outputs.setOutput48(no_output_state);

    outputs.setOutput49(no_output_state);
    outputs.setOutput50(no_output_state);
    outputs.setOutput51(no_output_state);
    outputs.setOutput52(no_output_state);

    outputs.setOutput53(no_output_state);
    outputs.setOutput54(no_output_state);
    outputs.setOutput55(no_output_state);
    outputs.setOutput56(no_output_state);

    outputs.setOutput57(no_output_state);
    outputs.setOutput58(no_output_state);
    outputs.setOutput59(no_output_state);
    outputs.setOutput60(no_output_state);

    outputs.setOutput61(no_output_state);
    outputs.setOutput62(no_output_state);

    return outputs;
}

_outputs InputsOutputs::getOutputs() const
{
    return outputs;
}

_outputs InputsOutputs::changeOutputs(char device_number, unsigned char cntrl)
{
    /// TODO сделать генерацию ошибок

    if (device_number == all_outputs)
    {
        if (cntrl == cntrl_on)
        {
            allOutputsOn();
        }

        if (cntrl == cntrl_off)
        {
            allOutputsOff();
        }
    }
    else
    {
        // говнокод - теперь придется делать switch и кучу одинаковых функций
        // массив был бы удобнее, но не так наглядно
        // еще моя реализация "жесткая", т.е. без пересборки не удастся изменить кол-во входов/выходов

        if (cntrl == cntrl_on)
        {
            oneOutputOn(device_number);
        }

        if (cntrl == cntrl_off)
        {
            oneOutputOff(device_number);
        }
    }
/*
    outputs.setOutput1(output_on);
    outputs.setOutput2(output_off);
    outputs.setOutput3(no_output_state);
    outputs.setOutput4(error_output);

    outputs.setOutput5(output_on);
    outputs.setOutput6(output_off);
    outputs.setOutput7(no_output_state);
    outputs.setOutput8(error_output);

    outputs.setOutput9(output_on);
    outputs.setOutput10(output_off);
    outputs.setOutput11(no_output_state);
    outputs.setOutput12(error_output);

    outputs.setOutput13(output_on);
    outputs.setOutput14(output_off);
    outputs.setOutput15(no_output_state);
    outputs.setOutput16(error_output);

    outputs.setOutput17(output_on);
    outputs.setOutput18(output_off);
    outputs.setOutput19(no_output_state);
    outputs.setOutput20(error_output);

    outputs.setOutput21(output_on);
    outputs.setOutput22(output_off);
    outputs.setOutput23(no_output_state);
    outputs.setOutput24(error_output);

    outputs.setOutput25(output_on);
    outputs.setOutput26(output_off);
    outputs.setOutput27(no_output_state);
    outputs.setOutput28(error_output);

    outputs.setOutput29(output_on);
    outputs.setOutput30(output_off);
    outputs.setOutput31(no_output_state);
    outputs.setOutput32(error_output);

    outputs.setOutput33(output_on);
    outputs.setOutput34(output_off);
    outputs.setOutput35(no_output_state);
    outputs.setOutput36(error_output);

    outputs.setOutput37(output_on);
    outputs.setOutput38(output_off);
    outputs.setOutput39(no_output_state);
    outputs.setOutput40(error_output);

    outputs.setOutput41(output_on);
    outputs.setOutput42(output_off);
    outputs.setOutput43(no_output_state);
    outputs.setOutput44(error_output);

    outputs.setOutput45(output_on);
    outputs.setOutput46(output_off);
    outputs.setOutput47(no_output_state);
    outputs.setOutput48(error_output);

    outputs.setOutput49(output_on);
    outputs.setOutput50(output_off);
    outputs.setOutput51(no_output_state);
    outputs.setOutput52(error_output);

    outputs.setOutput53(output_on);
    outputs.setOutput54(output_off);
    outputs.setOutput55(no_output_state);
    outputs.setOutput56(error_output);

    outputs.setOutput57(output_on);
    outputs.setOutput58(output_off);
    outputs.setOutput59(no_output_state);
    outputs.setOutput60(error_output);

    outputs.setOutput61(output_on);
    outputs.setOutput62(output_off);
*/
    return outputs;
}

void InputsOutputs::allOutputsOn()
{
    outputs.setOutput1(output_on);
    outputs.setOutput2(output_on);
    outputs.setOutput3(output_on);
    outputs.setOutput4(output_on);

    outputs.setOutput5(output_on);
    outputs.setOutput6(output_on);
    outputs.setOutput7(output_on);
    outputs.setOutput8(output_on);

    outputs.setOutput9(output_on);
    outputs.setOutput10(output_on);
    outputs.setOutput11(output_on);
    outputs.setOutput12(output_on);

    outputs.setOutput13(output_on);
    outputs.setOutput14(output_on);
    outputs.setOutput15(output_on);
    outputs.setOutput16(output_on);

    outputs.setOutput17(output_on);
    outputs.setOutput18(output_on);
    outputs.setOutput19(output_on);
    outputs.setOutput20(output_on);

    outputs.setOutput21(output_on);
    outputs.setOutput22(output_on);
    outputs.setOutput23(output_on);
    outputs.setOutput24(output_on);

    outputs.setOutput25(output_on);
    outputs.setOutput26(output_on);
    outputs.setOutput27(output_on);
    outputs.setOutput28(output_on);

    outputs.setOutput29(output_on);
    outputs.setOutput30(output_on);
    outputs.setOutput31(output_on);
    outputs.setOutput32(output_on);

    outputs.setOutput33(output_on);
    outputs.setOutput34(output_on);
    outputs.setOutput35(output_on);
    outputs.setOutput36(output_on);

    outputs.setOutput37(output_on);
    outputs.setOutput38(output_on);
    outputs.setOutput39(output_on);
    outputs.setOutput40(output_on);

    outputs.setOutput41(output_on);
    outputs.setOutput42(output_on);
    outputs.setOutput43(output_on);
    outputs.setOutput44(output_on);

    outputs.setOutput45(output_on);
    outputs.setOutput46(output_on);
    outputs.setOutput47(output_on);
    outputs.setOutput48(output_on);

    outputs.setOutput49(output_on);
    outputs.setOutput50(output_on);
    outputs.setOutput51(output_on);
    outputs.setOutput52(output_on);

    outputs.setOutput53(output_on);
    outputs.setOutput54(output_on);
    outputs.setOutput55(output_on);
    outputs.setOutput56(output_on);

    outputs.setOutput57(output_on);
    outputs.setOutput58(output_on);
    outputs.setOutput59(output_on);
    outputs.setOutput60(output_on);

    outputs.setOutput61(output_on);
    outputs.setOutput62(output_on);
}

void InputsOutputs::allOutputsOff()
{
    outputs.setOutput1(output_off);
    outputs.setOutput2(output_off);
    outputs.setOutput3(output_off);
    outputs.setOutput4(output_off);

    outputs.setOutput5(output_off);
    outputs.setOutput6(output_off);
    outputs.setOutput7(output_off);
    outputs.setOutput8(output_off);

    outputs.setOutput9(output_off);
    outputs.setOutput10(output_off);
    outputs.setOutput11(output_off);
    outputs.setOutput12(output_off);

    outputs.setOutput13(output_off);
    outputs.setOutput14(output_off);
    outputs.setOutput15(output_off);
    outputs.setOutput16(output_off);

    outputs.setOutput17(output_off);
    outputs.setOutput18(output_off);
    outputs.setOutput19(output_off);
    outputs.setOutput20(output_off);

    outputs.setOutput21(output_off);
    outputs.setOutput22(output_off);
    outputs.setOutput23(output_off);
    outputs.setOutput24(output_off);

    outputs.setOutput25(output_off);
    outputs.setOutput26(output_off);
    outputs.setOutput27(output_off);
    outputs.setOutput28(output_off);

    outputs.setOutput29(output_off);
    outputs.setOutput30(output_off);
    outputs.setOutput31(output_off);
    outputs.setOutput32(output_off);

    outputs.setOutput33(output_off);
    outputs.setOutput34(output_off);
    outputs.setOutput35(output_off);
    outputs.setOutput36(output_off);

    outputs.setOutput37(output_off);
    outputs.setOutput38(output_off);
    outputs.setOutput39(output_off);
    outputs.setOutput40(output_off);

    outputs.setOutput41(output_off);
    outputs.setOutput42(output_off);
    outputs.setOutput43(output_off);
    outputs.setOutput44(output_off);

    outputs.setOutput45(output_off);
    outputs.setOutput46(output_off);
    outputs.setOutput47(output_off);
    outputs.setOutput48(output_off);

    outputs.setOutput49(output_off);
    outputs.setOutput50(output_off);
    outputs.setOutput51(output_off);
    outputs.setOutput52(output_off);

    outputs.setOutput53(output_off);
    outputs.setOutput54(output_off);
    outputs.setOutput55(output_off);
    outputs.setOutput56(output_off);

    outputs.setOutput57(output_off);
    outputs.setOutput58(output_off);
    outputs.setOutput59(output_off);
    outputs.setOutput60(output_off);

    outputs.setOutput61(output_off);
    outputs.setOutput62(output_off);
}

void InputsOutputs::oneOutputOn(char device_number)
{
    /// TODO проверь - точно такие коды устройств?
    switch (device_number)
    {
    case 0x01:
        outputs.setOutput1(output_on);
        break;
    case 0x02:
        outputs.setOutput2(output_on);
        break;
    case 0x03:
        outputs.setOutput3(output_on);
        break;
    case 0x04:
        outputs.setOutput4(output_on);
        break;
    case 0x05:
        outputs.setOutput5(output_on);
        break;
    case 0x06:
        outputs.setOutput6(output_on);
        break;
    case 0x07:
        outputs.setOutput7(output_on);
        break;
    case 0x08:
        outputs.setOutput8(output_on);
        break;
    case 0x09:
        outputs.setOutput9(output_on);
        break;
    case 0x0a:
        outputs.setOutput10(output_on);
        break;
    case 0x0b:
        outputs.setOutput11(output_on);
        break;
    case 0x0c:
        outputs.setOutput12(output_on);
        break;
    case 0x0d:
        outputs.setOutput13(output_on);
        break;
    case 0x0e:
        outputs.setOutput14(output_on);
        break;
    case 0x0f:
        outputs.setOutput15(output_on);
        break;
    case 0x10:
        outputs.setOutput16(output_on);
        break;
    case 0x17:
        outputs.setOutput17(output_on);
        break;
    case 0x18:
        outputs.setOutput18(output_on);
        break;
    case 0x19:
        outputs.setOutput19(output_on);
        break;
    case 0x20:
        outputs.setOutput20(output_on);
        break;
    case 0x21:
        outputs.setOutput21(output_on);
        break;
    case 0x22:
        outputs.setOutput22(output_on);
        break;
    case 0x23:
        outputs.setOutput23(output_on);
        break;
    case 0x24:
        outputs.setOutput24(output_on);
        break;
    case 0x25:
        outputs.setOutput25(output_on);
        break;
    case 0x26:
        outputs.setOutput26(output_on);
        break;
    case 0x27:
        outputs.setOutput27(output_on);
        break;
    case 0x28:
        outputs.setOutput28(output_on);
        break;
    case 0x29:
        outputs.setOutput29(output_on);
        break;
    case 0x30:
        outputs.setOutput30(output_on);
        break;
    case 0x31:
        outputs.setOutput31(output_on);
        break;
    case 0x32:
        outputs.setOutput32(output_on);
        break;
    case 0x33:
        outputs.setOutput33(output_on);
        break;
    case 0x34:
        outputs.setOutput34(output_on);
        break;
    case 0x35:
        outputs.setOutput35(output_on);
        break;
    case 0x36:
        outputs.setOutput36(output_on);
        break;
    case 0x37:
        outputs.setOutput37(output_on);
        break;
    case 0x38:
        outputs.setOutput38(output_on);
        break;
    case 0x39:
        outputs.setOutput39(output_on);
        break;
    case 0x40:
        outputs.setOutput40(output_on);
        break;
    case 0x41:
        outputs.setOutput41(output_on);
        break;
    case 0x42:
        outputs.setOutput42(output_on);
        break;
    case 0x43:
        outputs.setOutput43(output_on);
        break;
    case 0x44:
        outputs.setOutput44(output_on);
        break;
    case 0x45:
        outputs.setOutput45(output_on);
        break;
    case 0x46:
        outputs.setOutput46(output_on);
        break;
    case 0x47:
        outputs.setOutput47(output_on);
        break;
    case 0x48:
        outputs.setOutput48(output_on);
        break;
    case 0x49:
        outputs.setOutput49(output_on);
        break;
    case 0x50:
        outputs.setOutput50(output_on);
        break;
    case 0x51:
        outputs.setOutput51(output_on);
        break;
    case 0x52:
        outputs.setOutput52(output_on);
        break;
    case 0x53:
        outputs.setOutput53(output_on);
        break;
    case 0x54:
        outputs.setOutput54(output_on);
        break;
    case 0x55:
        outputs.setOutput55(output_on);
        break;
    case 0x56:
        outputs.setOutput56(output_on);
        break;
    case 0x57:
        outputs.setOutput57(output_on);
        break;
    case 0x58:
        outputs.setOutput58(output_on);
        break;
    case 0x59:
        outputs.setOutput59(output_on);
        break;
    case 0x60:
        outputs.setOutput60(output_on);
        break;
    case 0x61:
        outputs.setOutput61(output_on);
        break;
    case 0x62:
        outputs.setOutput62(output_on);
        break;
    }
}

void InputsOutputs::oneOutputOff(char device_number)
{
    switch (device_number)
    {
    case 0x01:
        outputs.setOutput1(output_off);
        break;
    case 0x02:
        outputs.setOutput2(output_off);
        break;
    case 0x03:
        outputs.setOutput3(output_off);
        break;
    case 0x04:
        outputs.setOutput4(output_off);
        break;
    case 0x05:
        outputs.setOutput5(output_off);
        break;
    case 0x06:
        outputs.setOutput6(output_off);
        break;
    case 0x07:
        outputs.setOutput7(output_off);
        break;
    case 0x08:
        outputs.setOutput8(output_off);
        break;
    case 0x09:
        outputs.setOutput9(output_off);
        break;
    case 0x10:
        outputs.setOutput10(output_off);
        break;
    case 0x11:
        outputs.setOutput11(output_off);
        break;
    case 0x12:
        outputs.setOutput12(output_off);
        break;
    case 0x13:
        outputs.setOutput13(output_off);
        break;
    case 0x14:
        outputs.setOutput14(output_off);
        break;
    case 0x15:
        outputs.setOutput15(output_off);
        break;
    case 0x16:
        outputs.setOutput16(output_off);
        break;
    case 0x17:
        outputs.setOutput17(output_off);
        break;
    case 0x18:
        outputs.setOutput18(output_off);
        break;
    case 0x19:
        outputs.setOutput19(output_off);
        break;
    case 0x20:
        outputs.setOutput20(output_off);
        break;
    case 0x21:
        outputs.setOutput21(output_off);
        break;
    case 0x22:
        outputs.setOutput22(output_off);
        break;
    case 0x23:
        outputs.setOutput23(output_off);
        break;
    case 0x24:
        outputs.setOutput24(output_off);
        break;
    case 0x25:
        outputs.setOutput25(output_off);
        break;
    case 0x26:
        outputs.setOutput26(output_off);
        break;
    case 0x27:
        outputs.setOutput27(output_off);
        break;
    case 0x28:
        outputs.setOutput28(output_off);
        break;
    case 0x29:
        outputs.setOutput29(output_off);
        break;
    case 0x30:
        outputs.setOutput30(output_off);
        break;
    case 0x31:
        outputs.setOutput31(output_off);
        break;
    case 0x32:
        outputs.setOutput32(output_off);
        break;
    case 0x33:
        outputs.setOutput33(output_off);
        break;
    case 0x34:
        outputs.setOutput34(output_off);
        break;
    case 0x35:
        outputs.setOutput35(output_off);
        break;
    case 0x36:
        outputs.setOutput36(output_off);
        break;
    case 0x37:
        outputs.setOutput37(output_off);
        break;
    case 0x38:
        outputs.setOutput38(output_off);
        break;
    case 0x39:
        outputs.setOutput39(output_off);
        break;
    case 0x40:
        outputs.setOutput40(output_off);
        break;
    case 0x41:
        outputs.setOutput41(output_off);
        break;
    case 0x42:
        outputs.setOutput42(output_off);
        break;
    case 0x43:
        outputs.setOutput43(output_off);
        break;
    case 0x44:
        outputs.setOutput44(output_off);
        break;
    case 0x45:
        outputs.setOutput45(output_off);
        break;
    case 0x46:
        outputs.setOutput46(output_off);
        break;
    case 0x47:
        outputs.setOutput47(output_off);
        break;
    case 0x48:
        outputs.setOutput48(output_off);
        break;
    case 0x49:
        outputs.setOutput49(output_off);
        break;
    case 0x50:
        outputs.setOutput50(output_off);
        break;
    case 0x51:
        outputs.setOutput51(output_off);
        break;
    case 0x52:
        outputs.setOutput52(output_off);
        break;
    case 0x53:
        outputs.setOutput53(output_off);
        break;
    case 0x54:
        outputs.setOutput54(output_off);
        break;
    case 0x55:
        outputs.setOutput55(output_off);
        break;
    case 0x56:
        outputs.setOutput56(output_off);
        break;
    case 0x57:
        outputs.setOutput57(output_off);
        break;
    case 0x58:
        outputs.setOutput58(output_off);
        break;
    case 0x59:
        outputs.setOutput59(output_off);
        break;
    case 0x60:
        outputs.setOutput60(output_off);
        break;
    case 0x61:
        outputs.setOutput61(output_off);
        break;
    case 0x62:
        outputs.setOutput62(output_off);
        break;
    }
}

void InputsOutputs::setOutputs(const _outputs &value)
{
    outputs = value;
}

unsigned _inputs::input2() const
{
    return _input2;
}

void _inputs::setInput2(const unsigned &input2)
{
    _input2 = input2;
}

unsigned _inputs::input3() const
{
    return _input3;
}

void _inputs::setInput3(const unsigned &input3)
{
    _input3 = input3;
}

unsigned _inputs::input4() const
{
    return _input4;
}

void _inputs::setInput4(const unsigned &input4)
{
    _input4 = input4;
}

unsigned _inputs::input5() const
{
    return _input5;
}

void _inputs::setInput5(const unsigned &input5)
{
    _input5 = input5;
}

unsigned _inputs::input6() const
{
    return _input6;
}

void _inputs::setInput6(const unsigned &input6)
{
    _input6 = input6;
}

unsigned _inputs::input7() const
{
    return _input7;
}

void _inputs::setInput7(const unsigned &input7)
{
    _input7 = input7;
}

unsigned _inputs::input8() const
{
    return _input8;
}

void _inputs::setInput8(const unsigned &input8)
{
    _input8 = input8;
}

unsigned _inputs::input9() const
{
    return _input9;
}

void _inputs::setInput9(const unsigned &input9)
{
    _input9 = input9;
}

unsigned _inputs::input10() const
{
    return _input10;
}

void _inputs::setInput10(const unsigned &input10)
{
    _input10 = input10;
}

unsigned _inputs::input11() const
{
    return _input11;
}

void _inputs::setInput11(const unsigned &input11)
{
    _input11 = input11;
}

unsigned _inputs::input12() const
{
    return _input12;
}

void _inputs::setInput12(const unsigned &input12)
{
    _input12 = input12;
}

unsigned _inputs::input13() const
{
    return _input13;
}

void _inputs::setInput13(const unsigned &input13)
{
    _input13 = input13;
}

unsigned _inputs::input14() const
{
    return _input14;
}

void _inputs::setInput14(const unsigned &input14)
{
    _input14 = input14;
}

unsigned _inputs::input15() const
{
    return _input15;
}

void _inputs::setInput15(const unsigned &input15)
{
    _input15 = input15;
}

unsigned _inputs::input16() const
{
    return _input16;
}

void _inputs::setInput16(const unsigned &input16)
{
    _input16 = input16;
}

unsigned _inputs::input17() const
{
    return _input17;
}

void _inputs::setInput17(const unsigned &input17)
{
    _input17 = input17;
}

unsigned _inputs::input18() const
{
    return _input18;
}

void _inputs::setInput18(const unsigned &input18)
{
    _input18 = input18;
}

unsigned _inputs::input19() const
{
    return _input19;
}

void _inputs::setInput19(const unsigned &input19)
{
    _input19 = input19;
}

unsigned _inputs::input20() const
{
    return _input20;
}

void _inputs::setInput20(const unsigned &input20)
{
    _input20 = input20;
}

unsigned _inputs::input21() const
{
    return _input21;
}

void _inputs::setInput21(const unsigned &input21)
{
    _input21 = input21;
}

unsigned _inputs::input22() const
{
    return _input22;
}

void _inputs::setInput22(const unsigned &input22)
{
    _input22 = input22;
}

unsigned _inputs::input23() const
{
    return _input23;
}

void _inputs::setInput23(const unsigned &input23)
{
    _input23 = input23;
}

unsigned _inputs::input24() const
{
    return _input24;
}

void _inputs::setInput24(const unsigned &input24)
{
    _input24 = input24;
}

unsigned _inputs::input25() const
{
    return _input25;
}

void _inputs::setInput25(const unsigned &input25)
{
    _input25 = input25;
}

unsigned _inputs::input26() const
{
    return _input26;
}

void _inputs::setInput26(const unsigned &input26)
{
    _input26 = input26;
}

unsigned _inputs::input27() const
{
    return _input27;
}

void _inputs::setInput27(const unsigned &input27)
{
    _input27 = input27;
}

unsigned _inputs::input28() const
{
    return _input28;
}

void _inputs::setInput28(const unsigned &input28)
{
    _input28 = input28;
}

unsigned _inputs::input29() const
{
    return _input29;
}

void _inputs::setInput29(const unsigned &input29)
{
    _input29 = input29;
}

unsigned _inputs::input30() const
{
    return _input30;
}

void _inputs::setInput30(const unsigned &input30)
{
    _input30 = input30;
}

unsigned _inputs::input31() const
{
    return _input31;
}

void _inputs::setInput31(const unsigned &input31)
{
    _input31 = input31;
}

unsigned _inputs::input32() const
{
    return _input32;
}

void _inputs::setInput32(const unsigned &input32)
{
    _input32 = input32;
}

unsigned _inputs::input33() const
{
    return _input33;
}

void _inputs::setInput33(const unsigned &input33)
{
    _input33 = input33;
}

unsigned _inputs::input34() const
{
    return _input34;
}

void _inputs::setInput34(const unsigned &input34)
{
    _input34 = input34;
}

unsigned _inputs::input35() const
{
    return _input35;
}

void _inputs::setInput35(const unsigned &input35)
{
    _input35 = input35;
}

unsigned _inputs::input36() const
{
    return _input36;
}

void _inputs::setInput36(const unsigned &input36)
{
    _input36 = input36;
}

unsigned _inputs::input37() const
{
    return _input37;
}

void _inputs::setInput37(const unsigned &input37)
{
    _input37 = input37;
}

unsigned _inputs::input38() const
{
    return _input38;
}

void _inputs::setInput38(const unsigned &input38)
{
    _input38 = input38;
}

unsigned _inputs::input39() const
{
    return _input39;
}

void _inputs::setInput39(const unsigned &input39)
{
    _input39 = input39;
}

unsigned _inputs::input40() const
{
    return _input40;
}

void _inputs::setInput40(const unsigned &input40)
{
    _input40 = input40;
}

unsigned _inputs::input41() const
{
    return _input41;
}

void _inputs::setInput41(const unsigned &input41)
{
    _input41 = input41;
}

unsigned _inputs::input42() const
{
    return _input42;
}

void _inputs::setInput42(const unsigned &input42)
{
    _input42 = input42;
}

unsigned _inputs::input43() const
{
    return _input43;
}

void _inputs::setInput43(const unsigned &input43)
{
    _input43 = input43;
}

unsigned _inputs::input44() const
{
    return _input44;
}

void _inputs::setInput44(const unsigned &input44)
{
    _input44 = input44;
}

unsigned _inputs::input45() const
{
    return _input45;
}

void _inputs::setInput45(const unsigned &input45)
{
    _input45 = input45;
}

unsigned _inputs::input46() const
{
    return _input46;
}

void _inputs::setInput46(const unsigned &input46)
{
    _input46 = input46;
}

unsigned _inputs::input47() const
{
    return _input47;
}

void _inputs::setInput47(const unsigned &input47)
{
    _input47 = input47;
}

unsigned _inputs::input48() const
{
    return _input48;
}

void _inputs::setInput48(const unsigned &input48)
{
    _input48 = input48;
}

unsigned _inputs::input49() const
{
    return _input49;
}

void _inputs::setInput49(const unsigned &input49)
{
    _input49 = input49;
}

unsigned _inputs::input50() const
{
    return _input50;
}

void _inputs::setInput50(const unsigned &input50)
{
    _input50 = input50;
}

unsigned _inputs::input51() const
{
    return _input51;
}

void _inputs::setInput51(const unsigned &input51)
{
    _input51 = input51;
}

unsigned _inputs::input52() const
{
    return _input52;
}

void _inputs::setInput52(const unsigned &input52)
{
    _input52 = input52;
}

unsigned _inputs::input53() const
{
    return _input53;
}

void _inputs::setInput53(const unsigned &input53)
{
    _input53 = input53;
}

unsigned _inputs::input54() const
{
    return _input54;
}

void _inputs::setInput54(const unsigned &input54)
{
    _input54 = input54;
}

unsigned _inputs::input55() const
{
    return _input55;
}

void _inputs::setInput55(const unsigned &input55)
{
    _input55 = input55;
}

unsigned _inputs::input56() const
{
    return _input56;
}

void _inputs::setInput56(const unsigned &input56)
{
    _input56 = input56;
}

unsigned _inputs::input57() const
{
    return _input57;
}

void _inputs::setInput57(const unsigned &input57)
{
    _input57 = input57;
}

unsigned _inputs::input58() const
{
    return _input58;
}

void _inputs::setInput58(const unsigned &input58)
{
    _input58 = input58;
}

unsigned _inputs::input59() const
{
    return _input59;
}

void _inputs::setInput59(const unsigned &input59)
{
    _input59 = input59;
}

unsigned _inputs::input60() const
{
    return _input60;
}

void _inputs::setInput60(const unsigned &input60)
{
    _input60 = input60;
}

unsigned _inputs::input61() const
{
    return _input61;
}

void _inputs::setInput61(const unsigned &input61)
{
    _input61 = input61;
}

unsigned _inputs::input62() const
{
    return _input62;
}

void _inputs::setInput62(const unsigned &input62)
{
    _input62 = input62;
}

unsigned _inputs::input63() const
{
    return _input63;
}

void _inputs::setInput63(const unsigned &input63)
{
    _input63 = input63;
}

unsigned _inputs::input64() const
{
    return _input64;
}

void _inputs::setInput64(const unsigned &input64)
{
    _input64 = input64;
}

unsigned _inputs::input65() const
{
    return _input65;
}

void _inputs::setInput65(const unsigned &input65)
{
    _input65 = input65;
}

unsigned _inputs::input66() const
{
    return _input66;
}

void _inputs::setInput66(const unsigned &input66)
{
    _input66 = input66;
}

unsigned _inputs::input67() const
{
    return _input67;
}

void _inputs::setInput67(const unsigned &input67)
{
    _input67 = input67;
}

unsigned _inputs::input68() const
{
    return _input68;
}

void _inputs::setInput68(const unsigned &input68)
{
    _input68 = input68;
}

unsigned _inputs::input69() const
{
    return _input69;
}

void _inputs::setInput69(const unsigned &input69)
{
    _input69 = input69;
}

unsigned _inputs::input70() const
{
    return _input70;
}

void _inputs::setInput70(const unsigned &input70)
{
    _input70 = input70;
}

unsigned _inputs::input71() const
{
    return _input71;
}

void _inputs::setInput71(const unsigned &input71)
{
    _input71 = input71;
}

unsigned _inputs::input72() const
{
    return _input72;
}

void _inputs::setInput72(const unsigned &input72)
{
    _input72 = input72;
}

unsigned _inputs::input73() const
{
    return _input73;
}

void _inputs::setInput73(const unsigned &input73)
{
    _input73 = input73;
}

unsigned _inputs::input74() const
{
    return _input74;
}

void _inputs::setInput74(const unsigned &input74)
{
    _input74 = input74;
}

unsigned _inputs::input75() const
{
    return _input75;
}

void _inputs::setInput75(const unsigned &input75)
{
    _input75 = input75;
}

unsigned _inputs::input76() const
{
    return _input76;
}

void _inputs::setInput76(const unsigned &input76)
{
    _input76 = input76;
}

unsigned _inputs::input77() const
{
    return _input77;
}

void _inputs::setInput77(const unsigned &input77)
{
    _input77 = input77;
}

unsigned _inputs::input78() const
{
    return _input78;
}

void _inputs::setInput78(const unsigned &input78)
{
    _input78 = input78;
}

unsigned _inputs::input79() const
{
    return _input79;
}

void _inputs::setInput79(const unsigned &input79)
{
    _input79 = input79;
}

unsigned _inputs::input80() const
{
    return _input80;
}

void _inputs::setInput80(const unsigned &input80)
{
    _input80 = input80;
}

unsigned _inputs::input81() const
{
    return _input81;
}

void _inputs::setInput81(const unsigned &input81)
{
    _input81 = input81;
}

unsigned _inputs::input82() const
{
    return _input82;
}

void _inputs::setInput82(const unsigned &input82)
{
    _input82 = input82;
}

unsigned _inputs::input83() const
{
    return _input83;
}

void _inputs::setInput83(const unsigned &input83)
{
    _input83 = input83;
}

unsigned _inputs::input84() const
{
    return _input84;
}

void _inputs::setInput84(const unsigned &input84)
{
    _input84 = input84;
}

unsigned _inputs::input85() const
{
    return _input85;
}

void _inputs::setInput85(const unsigned &input85)
{
    _input85 = input85;
}

unsigned _inputs::input86() const
{
    return _input86;
}

void _inputs::setInput86(const unsigned &input86)
{
    _input86 = input86;
}

unsigned _inputs::input87() const
{
    return _input87;
}

void _inputs::setInput87(const unsigned &input87)
{
    _input87 = input87;
}

unsigned _inputs::input88() const
{
    return _input88;
}

void _inputs::setInput88(const unsigned &input88)
{
    _input88 = input88;
}

unsigned _inputs::input89() const
{
    return _input89;
}

void _inputs::setInput89(const unsigned &input89)
{
    _input89 = input89;
}

unsigned _inputs::input90() const
{
    return _input90;
}

void _inputs::setInput90(const unsigned &input90)
{
    _input90 = input90;
}

unsigned _inputs::input91() const
{
    return _input91;
}

void _inputs::setInput91(const unsigned &input91)
{
    _input91 = input91;
}

unsigned _inputs::input92() const
{
    return _input92;
}

void _inputs::setInput92(const unsigned &input92)
{
    _input92 = input92;
}

unsigned _inputs::input93() const
{
    return _input93;
}

void _inputs::setInput93(const unsigned &input93)
{
    _input93 = input93;
}

unsigned _inputs::input94() const
{
    return _input94;
}

void _inputs::setInput94(const unsigned &input94)
{
    _input94 = input94;
}

unsigned _inputs::input95() const
{
    return _input95;
}

void _inputs::setInput95(const unsigned &input95)
{
    _input95 = input95;
}

unsigned _inputs::input96() const
{
    return _input96;
}

void _inputs::setInput96(const unsigned &input96)
{
    _input96 = input96;
}

unsigned _inputs::input97() const
{
    return _input97;
}

void _inputs::setInput97(const unsigned &input97)
{
    _input97 = input97;
}

unsigned _inputs::input98() const
{
    return _input98;
}

void _inputs::setInput98(const unsigned &input98)
{
    _input98 = input98;
}

unsigned _inputs::input99() const
{
    return _input99;
}

void _inputs::setInput99(const unsigned &input99)
{
    _input99 = input99;
}

unsigned _inputs::input100() const
{
    return _input100;
}

void _inputs::setInput100(const unsigned &input100)
{
    _input100 = input100;
}

unsigned _inputs::input101() const
{
    return _input101;
}

void _inputs::setInput101(const unsigned &input101)
{
    _input101 = input101;
}

unsigned _inputs::input102() const
{
    return _input102;
}

void _inputs::setInput102(const unsigned &input102)
{
    _input102 = input102;
}

unsigned _inputs::input103() const
{
    return _input103;
}

void _inputs::setInput103(const unsigned &input103)
{
    _input103 = input103;
}

unsigned _inputs::input104() const
{
    return _input104;
}

void _inputs::setInput104(const unsigned &input104)
{
    _input104 = input104;
}

unsigned _inputs::input105() const
{
    return _input105;
}

void _inputs::setInput105(const unsigned &input105)
{
    _input105 = input105;
}

unsigned _inputs::input106() const
{
    return _input106;
}

void _inputs::setInput106(const unsigned &input106)
{
    _input106 = input106;
}

unsigned _inputs::input107() const
{
    return _input107;
}

void _inputs::setInput107(const unsigned &input107)
{
    _input107 = input107;
}

unsigned _inputs::input108() const
{
    return _input108;
}

void _inputs::setInput108(const unsigned &input108)
{
    _input108 = input108;
}

unsigned _inputs::input109() const
{
    return _input109;
}

void _inputs::setInput109(const unsigned &input109)
{
    _input109 = input109;
}

unsigned _inputs::input110() const
{
    return _input110;
}

void _inputs::setInput110(const unsigned &input110)
{
    _input110 = input110;
}

unsigned _inputs::input111() const
{
    return _input111;
}

void _inputs::setInput111(const unsigned &input111)
{
    _input111 = input111;
}

unsigned _inputs::input112() const
{
    return _input112;
}

void _inputs::setInput112(const unsigned &input112)
{
    _input112 = input112;
}

unsigned _inputs::input113() const
{
    return _input113;
}

void _inputs::setInput113(const unsigned &input113)
{
    _input113 = input113;
}

unsigned _inputs::input114() const
{
    return _input114;
}

void _inputs::setInput114(const unsigned &input114)
{
    _input114 = input114;
}

unsigned _inputs::input115() const
{
    return _input115;
}

void _inputs::setInput115(const unsigned &input115)
{
    _input115 = input115;
}

unsigned _inputs::input116() const
{
    return _input116;
}

void _inputs::setInput116(const unsigned &input116)
{
    _input116 = input116;
}

unsigned _inputs::input117() const
{
    return _input117;
}

void _inputs::setInput117(const unsigned &input117)
{
    _input117 = input117;
}

unsigned _inputs::input118() const
{
    return _input118;
}

void _inputs::setInput118(const unsigned &input118)
{
    _input118 = input118;
}

unsigned _inputs::input119() const
{
    return _input119;
}

void _inputs::setInput119(const unsigned &input119)
{
    _input119 = input119;
}

unsigned _inputs::input120() const
{
    return _input120;
}

void _inputs::setInput120(const unsigned &input120)
{
    _input120 = input120;
}

unsigned _inputs::input121() const
{
    return _input121;
}

void _inputs::setInput121(const unsigned &input121)
{
    _input121 = input121;
}

unsigned _inputs::input122() const
{
    return _input122;
}

void _inputs::setInput122(const unsigned &input122)
{
    _input122 = input122;
}

unsigned _inputs::input123() const
{
    return _input123;
}

void _inputs::setInput123(const unsigned &input123)
{
    _input123 = input123;
}

unsigned _inputs::input124() const
{
    return _input124;
}

void _inputs::setInput124(const unsigned &input124)
{
    _input124 = input124;
}

unsigned _inputs::input1() const
{
    return _input1;
}

void _inputs::setInput1(const unsigned &input1)
{
    _input1 = input1;
}

unsigned _outputs::output2() const
{
    return _output2;
}

void _outputs::setOutput2(const unsigned &output2)
{
    _output2 = output2;
}

unsigned _outputs::output3() const
{
    return _output3;
}

void _outputs::setOutput3(const unsigned &output3)
{
    _output3 = output3;
}

unsigned _outputs::output4() const
{
    return _output4;
}

void _outputs::setOutput4(const unsigned &output4)
{
    _output4 = output4;
}

unsigned _outputs::output5() const
{
    return _output5;
}

void _outputs::setOutput5(const unsigned &output5)
{
    _output5 = output5;
}

unsigned _outputs::output6() const
{
    return _output6;
}

void _outputs::setOutput6(const unsigned &output6)
{
    _output6 = output6;
}

unsigned _outputs::output7() const
{
    return _output7;
}

void _outputs::setOutput7(const unsigned &output7)
{
    _output7 = output7;
}

unsigned _outputs::output8() const
{
    return _output8;
}

void _outputs::setOutput8(const unsigned &output8)
{
    _output8 = output8;
}

unsigned _outputs::output9() const
{
    return _output9;
}

void _outputs::setOutput9(const unsigned &output9)
{
    _output9 = output9;
}

unsigned _outputs::output10() const
{
    return _output10;
}

void _outputs::setOutput10(const unsigned &output10)
{
    _output10 = output10;
}

unsigned _outputs::output11() const
{
    return _output11;
}

void _outputs::setOutput11(const unsigned &output11)
{
    _output11 = output11;
}

unsigned _outputs::output12() const
{
    return _output12;
}

void _outputs::setOutput12(const unsigned &output12)
{
    _output12 = output12;
}

unsigned _outputs::output13() const
{
    return _output13;
}

void _outputs::setOutput13(const unsigned &output13)
{
    _output13 = output13;
}

unsigned _outputs::output14() const
{
    return _output14;
}

void _outputs::setOutput14(const unsigned &output14)
{
    _output14 = output14;
}

unsigned _outputs::output15() const
{
    return _output15;
}

void _outputs::setOutput15(const unsigned &output15)
{
    _output15 = output15;
}

unsigned _outputs::output16() const
{
    return _output16;
}

void _outputs::setOutput16(const unsigned &output16)
{
    _output16 = output16;
}

unsigned _outputs::output17() const
{
    return _output17;
}

void _outputs::setOutput17(const unsigned &output17)
{
    _output17 = output17;
}

unsigned _outputs::output18() const
{
    return _output18;
}

void _outputs::setOutput18(const unsigned &output18)
{
    _output18 = output18;
}

unsigned _outputs::output19() const
{
    return _output19;
}

void _outputs::setOutput19(const unsigned &output19)
{
    _output19 = output19;
}

unsigned _outputs::output20() const
{
    return _output20;
}

void _outputs::setOutput20(const unsigned &output20)
{
    _output20 = output20;
}

unsigned _outputs::output21() const
{
    return _output21;
}

void _outputs::setOutput21(const unsigned &output21)
{
    _output21 = output21;
}

unsigned _outputs::output22() const
{
    return _output22;
}

void _outputs::setOutput22(const unsigned &output22)
{
    _output22 = output22;
}

unsigned _outputs::output23() const
{
    return _output23;
}

void _outputs::setOutput23(const unsigned &output23)
{
    _output23 = output23;
}

unsigned _outputs::output24() const
{
    return _output24;
}

void _outputs::setOutput24(const unsigned &output24)
{
    _output24 = output24;
}

unsigned _outputs::output25() const
{
    return _output25;
}

void _outputs::setOutput25(const unsigned &output25)
{
    _output25 = output25;
}

unsigned _outputs::output26() const
{
    return _output26;
}

void _outputs::setOutput26(const unsigned &output26)
{
    _output26 = output26;
}

unsigned _outputs::output27() const
{
    return _output27;
}

void _outputs::setOutput27(const unsigned &output27)
{
    _output27 = output27;
}

unsigned _outputs::output28() const
{
    return _output28;
}

void _outputs::setOutput28(const unsigned &output28)
{
    _output28 = output28;
}

unsigned _outputs::output29() const
{
    return _output29;
}

void _outputs::setOutput29(const unsigned &output29)
{
    _output29 = output29;
}

unsigned _outputs::output30() const
{
    return _output30;
}

void _outputs::setOutput30(const unsigned &output30)
{
    _output30 = output30;
}

unsigned _outputs::output31() const
{
    return _output31;
}

void _outputs::setOutput31(const unsigned &output31)
{
    _output31 = output31;
}

unsigned _outputs::output32() const
{
    return _output32;
}

void _outputs::setOutput32(const unsigned &output32)
{
    _output32 = output32;
}

unsigned _outputs::output33() const
{
    return _output33;
}

void _outputs::setOutput33(const unsigned &output33)
{
    _output33 = output33;
}

unsigned _outputs::output34() const
{
    return _output34;
}

void _outputs::setOutput34(const unsigned &output34)
{
    _output34 = output34;
}

unsigned _outputs::output35() const
{
    return _output35;
}

void _outputs::setOutput35(const unsigned &output35)
{
    _output35 = output35;
}

unsigned _outputs::output36() const
{
    return _output36;
}

void _outputs::setOutput36(const unsigned &output36)
{
    _output36 = output36;
}

unsigned _outputs::output37() const
{
    return _output37;
}

void _outputs::setOutput37(const unsigned &output37)
{
    _output37 = output37;
}

unsigned _outputs::output38() const
{
    return _output38;
}

void _outputs::setOutput38(const unsigned &output38)
{
    _output38 = output38;
}

unsigned _outputs::output39() const
{
    return _output39;
}

void _outputs::setOutput39(const unsigned &output39)
{
    _output39 = output39;
}

unsigned _outputs::output40() const
{
    return _output40;
}

void _outputs::setOutput40(const unsigned &output40)
{
    _output40 = output40;
}

unsigned _outputs::output41() const
{
    return _output41;
}

void _outputs::setOutput41(const unsigned &output41)
{
    _output41 = output41;
}

unsigned _outputs::output42() const
{
    return _output42;
}

void _outputs::setOutput42(const unsigned &output42)
{
    _output42 = output42;
}

unsigned _outputs::output43() const
{
    return _output43;
}

void _outputs::setOutput43(const unsigned &output43)
{
    _output43 = output43;
}

unsigned _outputs::output44() const
{
    return _output44;
}

void _outputs::setOutput44(const unsigned &output44)
{
    _output44 = output44;
}

unsigned _outputs::output45() const
{
    return _output45;
}

void _outputs::setOutput45(const unsigned &output45)
{
    _output45 = output45;
}

unsigned _outputs::output46() const
{
    return _output46;
}

void _outputs::setOutput46(const unsigned &output46)
{
    _output46 = output46;
}

unsigned _outputs::output47() const
{
    return _output47;
}

void _outputs::setOutput47(const unsigned &output47)
{
    _output47 = output47;
}

unsigned _outputs::output48() const
{
    return _output48;
}

void _outputs::setOutput48(const unsigned &output48)
{
    _output48 = output48;
}

unsigned _outputs::output49() const
{
    return _output49;
}

void _outputs::setOutput49(const unsigned &output49)
{
    _output49 = output49;
}

unsigned _outputs::output50() const
{
    return _output50;
}

void _outputs::setOutput50(const unsigned &output50)
{
    _output50 = output50;
}

unsigned _outputs::output51() const
{
    return _output51;
}

void _outputs::setOutput51(const unsigned &output51)
{
    _output51 = output51;
}

unsigned _outputs::output52() const
{
    return _output52;
}

void _outputs::setOutput52(const unsigned &output52)
{
    _output52 = output52;
}

unsigned _outputs::output53() const
{
    return _output53;
}

void _outputs::setOutput53(const unsigned &output53)
{
    _output53 = output53;
}

unsigned _outputs::output54() const
{
    return _output54;
}

void _outputs::setOutput54(const unsigned &output54)
{
    _output54 = output54;
}

unsigned _outputs::output55() const
{
    return _output55;
}

void _outputs::setOutput55(const unsigned &output55)
{
    _output55 = output55;
}

unsigned _outputs::output56() const
{
    return _output56;
}

void _outputs::setOutput56(const unsigned &output56)
{
    _output56 = output56;
}

unsigned _outputs::output57() const
{
    return _output57;
}

void _outputs::setOutput57(const unsigned &output57)
{
    _output57 = output57;
}

unsigned _outputs::output58() const
{
    return _output58;
}

void _outputs::setOutput58(const unsigned &output58)
{
    _output58 = output58;
}

unsigned _outputs::output59() const
{
    return _output59;
}

void _outputs::setOutput59(const unsigned &output59)
{
    _output59 = output59;
}

unsigned _outputs::output60() const
{
    return _output60;
}

void _outputs::setOutput60(const unsigned &output60)
{
    _output60 = output60;
}

unsigned _outputs::output61() const
{
    return _output61;
}

void _outputs::setOutput61(const unsigned &output61)
{
    _output61 = output61;
}

unsigned _outputs::output62() const
{
    return _output62;
}

void _outputs::setOutput62(const unsigned &output62)
{
    _output62 = output62;
}

unsigned _outputs::output1() const
{
    return _output1;
}

void _outputs::setOutput1(const unsigned &output1)
{
    _output1 = output1;
}
